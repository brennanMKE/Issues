...
public final class FolderBookmarkService {
    ...
    public nonisolated static func folderId(for bookmarkData: Data) -> String {
        let digest = SHA256.hash(data: bookmarkData)
        return digest.prefix(8).map { String(format: "%02x", $0) }.joined()
    }

    public private(set) var remembered: [RememberedFolder] = []
    ...
    public func presentOpenPanel() -> URL? {
    ...
    public func remember(url: URL) throws {
    ...
    public func resolve(_ folder: RememberedFolder) throws -> URL {
        var stale = false
        let url: URL
        do {
            url = try URL(
    ...
    public func forget(_ folder: RememberedFolder) {
