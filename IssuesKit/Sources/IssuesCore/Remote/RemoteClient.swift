...
public protocol RemoteClientProtocol: Sendable {
...
public final class URLSessionRemoteClient: RemoteClientProtocol, @unchecked Sendable {
    ...
    public init(host: String, port: UInt16, token: String, fingerprint: String, session: URLSession? = nil) {
        self.host = host
        self.port = port
        self.token = token
    ...
    public convenience init(host: String, port: UInt16, token: String, session: URLSession = .shared) {
        self.init(host: host, port: port, token: token, fingerprint: "", session: session)
    }

    // MARK: - Endpoints

    public func fetchFolder(id: String) async throws -> FolderInfo {
        let url = try makeURL("/v1/folders/\(id)")
        return try await get(url, kind: .folder)
    }

    public func fetchFolders() async throws -> [FolderInfo] {
        let url = try makeURL("/v1/folders")
        return try await get(url, kind: .folder)
    }

    public func fetchIssues(folderId: String) async throws -> [IssueMetadata] {
        let url = try makeURL("/v1/folders/\(folderId)/issues")
        return try await get(url, kind: .folder)
    }

    public func fetchIssueDetail(folderId: String, id: String) async throws -> IssueDetail {
        let url = try makeURL("/v1/folders/\(folderId)/issues/\(id)")
        return try await get(url, kind: .issue)
    }

    public func fetchAttachmentData(folderId: String, issueId: String, name: String) async throws -> Data {
