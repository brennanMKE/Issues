...
//
// `issues-dashboard` CLI entry point. Mirrors IssuesCLIRunner for argument
// parsing, path resolution, and exit-code contract. On success, constructs
// an IssueStore, starts the 1 s poll loop, and hands off to SwiftTUI's
// Application run loop (which blocks until Ctrl-C).

import Foundation
import IssuesCore
import SwiftTUI

public enum IssuesDashboardCLIRunner {
    ...
    private static func run() async throws {
        ...
        let store = DashboardIssueStore(folderURL: folderURL, interval: 1.0)
        store.start()
