...
## Goal — Safari's behavior

![Safari demonstrating live tab reorder](0021/safari-tab-reordering.gif)

[Higher-quality .mov](0021/safari-tab-reordering.mov) · [Plan and reference implementation](0021/safari_tab_reordering.md)

## Timeline of attempts (with recordings)

This issue went through seven iterations before landing. Each attempt revealed a deeper bug that the next had to fix. Recordings are embedded inline below — they render as animated GIFs in any markdown renderer (including Issues.app's own Textual sheet via #0001 and #0018).

### Attempt 1 — `ZStack` + `.offset(x:)` (commit `40e25ab`)

Followed the reference plan literally. Chips were placed by `.offset(x: idx * stride)` inside a `ZStack(alignment: .leading)`; the dragged chip's offset switched to `originalX + dragOffset`; the array was mutated mid-drag on each slot crossing.

**Result**: **broken hit testing**. `.offset(x:)` translates only the rendered view, not the layout frame. All chips' tap zones stayed at `x = 0`. Tap-to-activate only worked on whichever chip the array happened to put first; drags only initiated from the leading edge regardless of the visible chip layout.

### Attempt 2 — `HStack` with offset compensation (commit `8271bbd`)

Switched to a regular `HStack(spacing:)` so chips occupy their natural layout slots. Applied `.offset(x:)` only to the dragged chip with a compensation formula `(originalIndex - currentIdx) * stride + dragXOffset` so the chip stayed under the cursor while the array continued to reorder.

**Result**: tap-to-activate worked again, but the drag was visibly **jittery**. Each midpoint crossing flashed/snapped the chip between positions.

![Jittery reorder — chip flashes between positions on each midpoint crossing](0021/jittery-reordering.gif)

[Higher-quality .mov](0021/jittery-reordering.mov)

### Attempt 3 — disable animation on dragged chip + non-animated transaction (commit `2223974`)

Gated the per-chip slot-change spring on `!isDragging` and wrapped the mid-drag array mutation in `withTransaction(Transaction(animation: nil))` so SwiftUI wouldn't apply implicit animation across the array reorder.

**Result**: still wrong. Non-dragged chips drifted and ghosted before they should — the implementation kept mutating `tabs.tabs` mid-drag, so SwiftUI fought between HStack reflow and the offset compensation. Even with animations disabled, one-frame mismatches showed up.

![Ghosting — non-dragged chips drift before they should](0021/ghosting.gif)

[Higher-quality .mov](0021/ghosting.mov)

### Attempt 4 — phantom-slot placeholder model (commit `4d58c22`)

Stopped mutating `tabs.tabs` during the drag. The chip's original cell became a `Color.clear` "ghost" whose width animates between `draggedWidth` and `0` based on whether the phantom slot equals the original. A single `Color.clear` placeholder is inserted at the phantom-slot index in a derived `displayItems` array. The visible dragged chip is rendered as a separate floating overlay positioned by `originalSlotAnchor + dragXOffset`. Variable widths via `onGeometryChange`. Reorder fires once on `.onEnded`.

**Result**: phantom-slot threshold was **wrong** — used "dragged chip's center past neighbor's center" (one full stride of drag) instead of Safari's "the moment the cursor enters another tab's slot." Chips also began **expanding to fill the bar width** because of an internal `Spacer(minLength: 0)` in `TabChipView`.

### Attempt 5 — cursor-X threshold + intrinsic widths (commit `e404b4a`)

Switched the threshold to "cursor X enters a slot's `[anchorX, anchorX + width)` range." Removed the internal Spacer in `TabChipView` and pinned chips with `.fixedSize(horizontal: true, vertical: false)` plus a wrapping `.fixedSize` on the inner HStack.

**Result**: visible behavior much improved but still failing. **Drop didn't reliably cancel.** After release, the floating overlay and/or placeholder remained on screen. Mid-drag the cursor passing over neighbors triggered hover-revealed close (X) buttons, and the eventual mouse-up could **close the wrong tab**. Drag itself was **choppy**.

The smoking-gun log captured: a single drag attempt that produced exactly **one** `ON_CHANGED` event, then silence, no `DRAG_ENDED`:

```
DRAG_START id=140D26FC name=Issues originalSlot=1 anchor=123.0 draggedWidth=95.5
ON_CHANGED translation=-5.0 cursorBarX=176.5 newPhantom=1 currentPhantom=1
...
The drag silenced the moment the cursor left the dragged chip's hit area. Diagnosis: **the gesture's host view was being replaced mid-drag**. The cell switch (`if chip.id == draggingID { Color.clear } else { TabChipView }`) replaced the `TabChipView` that owned the gesture as soon as `draggingID` was set; `DragGesture` does not survive replacement of its host.

### Attempt 6 — gesture on a stable parent (commit `ebb37a5`)

Moved `DragGesture` from each `TabChipView` to the inner `HStack` of cells via `.simultaneousGesture(barDragGesture)`. The `HStack`'s identity does not change as cells re-render, so the gesture survives the entire drag. First-tick chip identification via a new `chipAtBarX(_:)` helper that hit-tests `value.startLocation.x` against cumulative chip anchors. `value.location.x` is now in bar coords directly.

**Result**: drag itself worked smoothly. 56 `ON_CHANGED` ticks per drag at ~1pt cursor steps. `PHANTOM_CHANGE` fired exactly when the cursor crossed into a neighbor's slot. `DRAG_ENDED → REORDER_COMMIT → DRAG_CLEANUP_DONE` on every release. No `CLOSE_BUTTON_HIT` events mid-drag — the parent gesture suppresses inner button hits during a drag.

But — with three tabs `[Issues, EurekaPlatforms, BucketKit]`, dragging the first or second tab to the **last** slot didn't stick. For the second tab it bounced back to its original index.

![Drag-to-last bug — Issues was dragged toward the trailing slot but landed in the middle / didn't stay](0021/last-position-bug.png)

### Final fix — off-by-one at the call site (commit `3aadf5a`)

`TabsModel.reorderWithoutPersisting(from:to:)` uses SwiftUI's `Array.move(fromOffsets:toOffset:)` semantics — `toOffset` is the index BEFORE which to insert in the **original** array, then it adjusts internally with `(destination > source) ? destination - 1 : destination` after the `remove`. The drag-end call site was passing the phantom slot `p` directly, which lands forward moves one slot short.

For `d = 0, p = 2` in `[T0, T1, T2]`:
- `remove(at: 0)` → `[T1, T2]`
- `insertIndex = 2 - 1 = 1`
- `insert at 1` → `[T1, T0, T2]` — wrong; user wanted `[T1, T2, T0]`.

For `d = 1, p = 2`: same shape — input array unchanged, the "won't stay / bounces back" symptom.

Adjust at the call site so the move-style `toOffset` is what gets passed:
...
Backward moves were unaffected (the function's adjustment is a no-op when `destination <= source`). The same adjustment was already in the prototype project's call site, which is why the prototype's earlier logs (all backward moves) didn't expose the bug; switching the prototype seed to match the user's 3-tab screenshot is what surfaced it. Net: +8 / -1 lines.

### Summary table

| # | Commit | Fix attempted | Why it failed |
|---|---|---|---|
| 1 | `40e25ab` | `ZStack` + `.offset(x:)` | Hit testing detached from rendering |
| 2 | `8271bbd` | `HStack` + offset compensation only on dragged chip | Spring animation fought cursor at each midpoint |
| 3 | `2223974` | Disable animation on dragged chip; non-animated transaction | Mid-drag array mutation still caused HStack-reflow ghosting |
| 4 | `4d58c22` | Phantom-slot placeholder; reorder once on release | Wrong threshold; chips flexed to fill bar |
| 5 | `e404b4a` | Cursor-X threshold; intrinsic widths via `.fixedSize` | Gesture host replaced mid-drag → drag died, leaks, choppiness |
| 6 | `ebb37a5` | Drag gesture on inner `HStack` (stable parent) + `chipAtBarX(_:)` | Forward-to-last drops landed one slot short |
| ✓ | `3aadf5a` | `(p > d) ? p + 1 : p` adjustment at the call site | — |

The complete in-house solution is **two surgical changes**: (a) attach the drag gesture to a parent that doesn't get torn down, (b) match the call-site index to the function's `Array.move` semantics. The phantom-slot model, ghost + placeholder, floating overlay, cursor-X threshold, `onGeometryChange` widths, and split persist pair from the earlier attempts all stayed.
...
## Follow-up — off-by-one when dragging to the last slot (commit `3aadf5a`)

After the gesture-host fix landed and the smooth-drag flow was verified, one residual bug appeared with three tabs `[Issues, EurekaPlatforms, BucketKit]`: dragging the first or second tab to the last slot didn't stick. For the second tab, it bounced back to its original index. Forward-to-last moves only.

### Root cause — call-site / function-contract mismatch

`TabsModel.reorderWithoutPersisting(from:to:)` uses SwiftUI's `Array.move(fromOffsets:toOffset:)` semantics — for forward moves, it expects `toOffset` to be the index BEFORE which to insert in the **original** array, then it does `(destination > source) ? destination - 1 : destination` after the `remove` to land the value at the right index. To move T0 past T2 in `[T0, T1, T2]` to the trailing position, the caller is supposed to pass `toOffset = 3`, not `2`.

The drag-end call site was passing the phantom slot `p` directly:

```swift
tabs.reorderWithoutPersisting(from: d, to: p)
```

With three tabs and `d = 0, p = 2`:
- `remove(at: 0)` → `[T1, T2]`
- `insertIndex = (2 > 0) ? 1 : 2` → 1
- `insert(T0, at: 1)` → `[T1, T0, T2]` — wrong; user wanted `[T1, T2, T0]`.

For `d = 1, p = 2`:
- `remove(at: 1)` → `[T0, T2]`
- `insertIndex = 1`
- `insert(T1, at: 1)` → `[T0, T1, T2]` — input unchanged. The "won't stay / bounces back" symptom.

Backward moves were unaffected because the function's `(destination > source)` branch is false and the `-1` adjustment becomes a no-op.

### Fix

Apply the move-style adjustment at the call site so the destination passed in matches what the function expects:
...
For `d = 0, p = 2` → `destination = 3` → `remove`, `insertIndex = 3 - 1 = 2`, `insert at 2` → `[T1, T2, T0]`. Correct.

The same adjustment was already in the prototype project's `Tabs/Views/TabBarView.swift`, which is why the prototype's earlier logs (all backward moves) didn't expose the bug. Adopting the prototype's seed `[Issues, EurekaPlatforms, BucketKit]` and trying the forward-to-last drag in the prototype confirmed it works there; porting the one-line adjustment to the main app's call site closes the loop.

Net diff for this fix: +8 / -1 lines in `Issues/Views/TabBarView.swift`.
