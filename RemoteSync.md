...
```
- Bucket has versioning + lifecycle (`NoncurrentVersionExpiration: 90 days`) for free rollback.
- Object metadata includes `etag-sha256` and `modified-by-user`.
- Server access via IAM role attached to the EC2 instance — no static credentials.
...
- **nginx**: terminates TLS on `:443`, proxies `/` and `/admin/*` to `127.0.0.1:8080`. WebSocket upgrade pass-through (`proxy_set_header Upgrade $http_upgrade; proxy_set_header Connection "upgrade"; proxy_read_timeout 1h;`).
...
- **Config (env)**: `BASE_URL`, `RP_ID`, `RP_DISPLAY_NAME`, `ADMIN_EMAIL`, `DATABASE_URL`, `S3_BUCKET`, `AWS_REGION`, `JWT_SIGNING_KEY`, `INVITE_SIGNING_KEY`, `SMTP_*` (for invite emails).
- **Email**: SES via the same IAM role. Templates are plain text + minimal HTML.
...
- **Storage: S3 for bytes, Postgres for metadata.** Single S3 bucket with versioning + lifecycle.
