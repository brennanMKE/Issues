...
# pushing the tag to origin so the user can review locally first.
#
# Usage:
...
if [[ -z "$VERSION" ]]; then
    print -u2 "error: could not extract MARKETING_VERSION from $XCCONFIG"
    exit 1
fi
...
git tag -a "$TAG" -m "Issues $VERSION"
