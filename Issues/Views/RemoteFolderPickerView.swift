...
final class RemoteFolderPickerModel {
    ...
    /// `URLSessionRemoteClient` default suits production.
    var client: RemoteClientProtocol = URLSessionRemoteClient()
    ...
    func validateHost() async {
        ...
        do {
            ...
            phase = .token(host: host, port: port)
        } catch RemoteClientError.transport(let detail) {
            inlineError = "Couldn't reach this host. (\(detail))"
        } catch RemoteClientError.httpStatus(let code) {
            inlineError = "Host returned status \(code)."
        } catch RemoteClientError.invalidURL {
            inlineError = "That host:port is not a valid address."
        } catch RemoteClientError.decoding(let detail) {
